...
- **Trade-off:** Schema Mục 12 gợi ý `linko-backend/`. Nhưng schema là *technical spec*, còn monorepo guide là *governance* — governance thắng.
...
- **Evidence:** `temp/verification.md` Section T1 — trích dẫn uv Docker docs: *"uv sync --locked — asserting the lockfile is up to date"*.
...
- **Evidence:** `temp/verification.md` Section F4 — trích dẫn Gemini deprecations page: *"Gemini API Shutdown Date: January 14, 2026"*.
...
- **Evidence:** `temp/verification.md` Section F1 — trích dẫn Alembic docs: *"Some free-standing constraint additions ... including PRIMARY KEY, EXCLUDE, CHECK; these are not necessarily implemented within the autogenerate detection system"*.
...
- **Evidence:** `temp/verification.md` Section F3 — trích dẫn SQLAlchemy 2.0 docs: *"server side default ... takes place even if we emit INSERT commands to the table from the SQL command line"*.
...
- **Evidence:** `temp/verification.md` Section F10 — trích dẫn issue gốc: *"Operator classes in index produces wrong SQL when using column labels"*.
